...
#if 1
{
    //added by dwelch to load a binary into sram and change the pc (r15)

    unsigned int ra;
    unsigned int rb;
    FILE *fpin;

    fpin=fopen("blinker.bin","rb");
    if(fpin==NULL)
    {
        printf("Error opening file blinker.bin\n");
        return(1);
    }
    ra=0x20000000;
    while(1)
    {
        if(fread(&rb,1,4,fpin)==0) break;
    ...
    for(ra=0x20000000;ra<0x20000010;ra++)
    {
    ...
    stlink_write_reg(sl, 0x20000050, 15);

    //let it fall through and allow the processor to run again
}
#endif
#if 0
